# Plantilla de Fábrica — Contrato de Locación

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Crear una plantilla pre-construida con las 33 cláusulas del contrato de locación estándar, disponible automáticamente al crear una agencia, editable por los agentes desde el generador de documentos.

**Architecture:** Se agrega un campo `source` al schema de `documentTemplate` para distinguir plantillas de fábrica de las creadas por usuarios. Una función `ensureDefaultTemplate(agencyId)` crea la plantilla si no existe, y se llama en el endpoint de creación de agencia.

**Tech Stack:** Drizzle ORM · PostgreSQL · Next.js Route Handler · TypeScript

---

## Archivos que se modifican o crean

| Acción | Archivo | Responsabilidad |
|--------|---------|-----------------|
| Modificar | `src/db/schema/document-template.ts` | Agregar campo `source` |
| Crear | `src/lib/document-templates/default-template.ts` | Función `ensureDefaultTemplate` con las 33 cláusulas |
| Modificar | `src/app/api/agency/route.ts` | Llamar `ensureDefaultTemplate` al crear la agencia |

---

## Task 1: Rama + campo `source` en el schema
...
```bash
git checkout -b feat/plantilla-contrato
```

- [ ] **Paso 2: Agregar el campo `source` al schema**

Abrir `src/db/schema/document-template.ts` y reemplazar el contenido completo con:
...
Resultado esperado: Drizzle crea una nueva migración que agrega la columna `source` con valor por defecto `"custom"` a la tabla `documentTemplate`. El `db:migrate` la aplica sin errores.

- [ ] **Paso 4: Commit**

```bash
git add src/db/schema/document-template.ts drizzle/
git commit -m "feat(schema): add source field to documentTemplate"
```

---

## Task 2: Función `ensureDefaultTemplate` con las 33 cláusulas

**Files:**
- Create: `src/lib/document-templates/default-template.ts`

Esta es la tarea principal. La función crea la plantilla completa si no existe para la agencia.

- [ ] **Paso 1: Crear el archivo `src/lib/document-templates/default-template.ts`**
...
Resultado esperado: sin errores de tipo. Si hay errores, el más probable es que `documentTemplate.source` no esté en el tipo inferido todavía — en ese caso confirmar que el schema fue guardado correctamente en el paso anterior.

- [ ] **Paso 3: Commit**

```bash
git add src/lib/document-templates/default-template.ts
git commit -m "feat(templates): add ensureDefaultTemplate with 33-clause rental contract"
```

---

## Task 3: Llamar `ensureDefaultTemplate` al crear la agencia

**Files:**
- Modify: `src/app/api/agency/route.ts`

- [ ] **Paso 1: Agregar el import en `src/app/api/agency/route.ts`**
...
- [ ] **Paso 2: Extraer el ID y llamar la función después del insert**

Buscar el bloque `else` del upsert (líneas ~100-106 del archivo actual) y reemplazarlo con:
...
- [ ] **Paso 3: Verificar que TypeScript no reporta errores**
...
```bash
git add src/app/api/agency/route.ts
git commit -m "feat(agency): call ensureDefaultTemplate on agency creation"
```
...
- [ ] **Paso 2: Forzar la creación de la plantilla para la agencia existente**

Como la agencia de Arce ya existe (no se va a re-crear), llamar la función directamente desde una ruta temporal o desde Drizzle Studio. La manera más simple es abrir `src/app/api/agency/route.ts`, encontrar el PATCH handler, y agregar temporalmente esta llamada al final del handler para agencias existentes, guardar, hacer un PATCH desde el browser, y luego remover.

Alternativa más limpia: abrir la consola de Node y correr:

```bash
bun -e "
import { db } from './src/db/index.ts';
import { agency } from './src/db/schema/index.ts';
import { ensureDefaultTemplate } from './src/lib/document-templates/default-template.ts';
const [ag] = await db.select().from(agency).limit(1);
console.log('Agency ID:', ag.id);
await ensureDefaultTemplate(ag.id);
console.log('Done');
process.exit(0);
"
```

Resultado esperado: imprime el ID de la agencia y "Done" sin errores.

- [ ] **Paso 3: Verificar en el generador de documentos**

Abrir `http://localhost:3000/generador-documentos` en el browser.

Resultado esperado: aparece la plantilla "Contrato de Locación — Modelo Arce" en la lista.

- [ ] **Paso 4: Abrir la plantilla y verificar las cláusulas**

Hacer clic en la plantilla. Verificar que:
- Aparecen 34 cláusulas (0 a 33)
- El encabezado muestra `[[apellido_locador]]`, `[[nombres_locador]]`, etc.
- La cláusula PRIMERA tiene las variables de propiedad y los `{{estado_*}}`
- La cláusula TERCERA tiene `[[fecha_inicio]]` y `[[fecha_fin]]`
- La cláusula CUARTA tiene `[[precio_inicial_letras]]` y `[[precio_inicial_formato]]`

- [ ] **Paso 5: Probar la previsualización con un contrato real**

Seleccionar un contrato existente en el selector de la plantilla. Verificar que las variables `[[...]]` se resuelven con los datos reales y que las variables `{{...}}` aparecen como campos editables.

- [ ] **Paso 6: Commit final**

```bash
git add .
git commit -m "chore: verify default template creation"
```
...
- El campo `source` acepta solo `"custom"` | `"factory"`. No agregar validación Zod en la API de templates porque ese campo no es editable por el usuario desde el frontend.
- La función `ensureDefaultTemplate` es idempotente: llamarla múltiples veces sobre la misma agencia solo crea la plantilla la primera vez.
- `{{duracion_letras}}` en la cláusula TERCERA es texto libre porque el sistema resuelve `[[duracion_texto]]` como número en letras en minúscula ("veinticuatro"), no como "DOS AÑOS". El agente lo completa a mano. Mejora futura: agregar variable `duracion_anios_texto`.
- Los `[[...]]` de fiadores 1-3 funcionan solo si el contrato tiene garantes vinculados. Si no tiene, el texto quedará vacío — el agente puede editar la cláusula 18 manualmente para ese contrato.
