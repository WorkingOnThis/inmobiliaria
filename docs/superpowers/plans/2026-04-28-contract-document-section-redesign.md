# Contract Document Section — Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the current clause list + modal editor with an inline accordion editor and a toggleable split-pane preview.

**Architecture:** Single component rewrite (`contract-document-section.tsx`). All state lives at the top level — one clause expanded at a time, editing state (`editTitle`, `editBody`, `editOverrides`) initialized on expand. Preview toggle switches between accordion layout and split-pane (compact sidebar + rendered preview). The variable popover is triggered from a compact preview row inside the expanded clause. `ContractClauseEditorModal` is deleted.

**Tech Stack:** React 19 · TanStack Query v5 · @dnd-kit/core + @dnd-kit/sortable · shadcn/ui · TypeScript

**Spec:** `docs/superpowers/specs/2026-04-28-contract-document-section-redesign.md`
...
| File | Change |
|---|---|
| `src/components/contracts/contract-document-section.tsx` | Full rewrite |
| `src/components/contracts/contract-clause-editor-modal.tsx` | Delete |

**Reused without changes:**
- `@/lib/document-templates/highlighted-body-textarea` → `HighlightedBodyTextarea`
- `@/lib/document-templates/variable-popover` → `VariablePopover`, `PopoverState`
- `@/lib/document-templates/render-segments` → `renderClauseBody`
- `@/lib/document-templates/ordinal-clause` → `clauseHeading`
- `@/db/schema/contract-clause` → `ContractClause`

---

### Task 1: Rewrite ContractDocumentSection

**Files:**
- Modify: `src/components/contracts/contract-document-section.tsx` (full rewrite)

- [ ] **Step 1: Replace the file with the new implementation**

Overwrite `src/components/contracts/contract-document-section.tsx` with:
...
Expected: no TypeScript errors. The old `ContractClauseEditorModal` import was removed from this file, so it won't cause a missing-module error here — but Task 2 will verify the entire project.

- [ ] **Step 3: Commit**

```bash
git add src/components/contracts/contract-document-section.tsx
git commit -m "feat(ui): redesign ContractDocumentSection — accordion editor + preview toggle"
```

---

### Task 2: Delete ContractClauseEditorModal and verify

**Files:**
- Delete: `src/components/contracts/contract-clause-editor-modal.tsx`

- [ ] **Step 1: Check that nothing else imports the modal**

```bash
grep -r "contract-clause-editor-modal" src/ --include="*.tsx" --include="*.ts"
```

Expected: no results (the new `contract-document-section.tsx` no longer imports it).

If any result appears, open that file and remove the import and usage before proceeding.
...
Expected: no errors. If there's a missing-module error referencing `contract-clause-editor-modal`, a file still imports it — find it with the grep from Step 1 and fix it.

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "chore: remove ContractClauseEditorModal (replaced by inline accordion)"
```
...
### Spec coverage check
- [x] Estado 1 — acordeón: click fila → expande con título + textarea + preview + guardar/cancelar → `SortableClauseRow`
- [x] Solo una cláusula expandida a la vez → `expandClause()` reemplaza el estado previo sin prompt
- [x] Drag handle y switch visibles en cláusula expandida → el header siempre muestra ambos
- [x] Cláusulas inactivas: tachadas, expandibles igual → condicional en `className` del título
- [x] `HighlightedBodyTextarea` con `overrides` → prop pasado desde `editOverrides`
- [x] Variable popover Ctrl+Click en preview → `onPopoverOpen` pasado a `renderClauseBody`
- [x] `fieldOverrides` persistidos en PATCH → `saveClause` envía `{ title, body, fieldOverrides }`
- [x] Estado 2 — preview toggle → `previewOpen` state, botón Eye/EyeOff
- [x] Sidebar compacto ~160px con títulos numerados → `w-40` con lista de botones
- [x] Cláusulas inactivas en sidebar: tachadas sin número → conditional en sidebar buttons
- [x] Cláusula seleccionada resaltada → `previewFocusId === clause.id`
- [x] Clic en sidebar → scroll suave → `scrollToClause()` con `scrollIntoView`
- [x] Preview renderizado con `renderClauseBody` + `clauseHeading` + `fieldOverrides` → sección preview
- [x] Botón cambia a "Cerrar preview" → `previewOpen ? "Cerrar preview" : "Vista previa"`
- [x] Abrir preview colapsa cualquier cláusula expandida → `openPreview()` llama `collapseClause()`
- [x] Imprimir disponible en ambos estados → botón Imprimir siempre visible en header
- [x] Eliminar `ContractClauseEditorModal` → Task 2
- [x] `localOrder` para DnD optimista → mantenido de la versión anterior
