...
export function TenantTabDocuments({ tenantName }: TenantTabDocumentsProps) {
  ...
  const [form, setForm] = useState({
    ...
    tipoCustom: "",
    vigenciaHasta: "",
    visiblePara: "staff" as "staff" | "inquilino" | "ambos",
    notasInternas: "",
  });
  ...
  return (
    <div className="p-7 flex flex-col gap-5">
      ...
      {documentos.length === 0 && (
        ...
        >
          <div className="size-11 rounded-[10px] flex items-center justify-center" style={{ background: "var(--surface-mid)" }}>
            <Upload size={20} className="text-muted-foreground" />
          </div>
          <div className="text-center">
            <p className="text-[0.82rem] font-semibold text-on-surface">Arrastrá archivos o hacé clic para subir</p>
            <p className="text-[0.72rem] text-muted-foreground mt-0.5">PDF, Word, imágenes — máx. 20 MB</p>
          </div>
        </div>
      )}
      ...
      {documentos.length > 0 && (
        <div className="bg-surface border border-border rounded-[10px] overflow-hidden">
          {documentos.map((doc, i) => (
            <div key={doc.id} className={cn("flex items-center gap-3 px-4 py-3 hover:bg-surface-mid transition-colors", i > 0 && "border-t border-border")}>
              ...
              <button className="text-muted-foreground hover:text-on-surface transition-colors"><X size={14} /></button>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-[3px] flex items-center justify-center px-4">
          <div className="bg-surface border border-border rounded-[12px] w-full max-w-[540px] overflow-hidden">
            ...
            <div className="px-6 py-5 flex flex-col gap-4">
              <div
                className={cn("border-2 border-dashed rounded-[10px] py-7 flex flex-col items-center gap-2 cursor-pointer transition-all", dragging ? "border-primary bg-primary-dim" : "border-border hover:border-primary/50")}
                onDragOver={(e) => { e.preventDefault(); setDragging(true); }}
                onDragLeave={() => setDragging(false)}
                onDrop={(e) => { e.preventDefault(); setDragging(false); const f = e.dataTransfer.files[0]; if (f) acceptFile(f); }}
                onClick={() => fileRef.current?.click()}
              >
                <input ref={fileRef} type="file" className="hidden" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp" onChange={(e) => { const f = e.target.files?.[0]; if (f) acceptFile(f); }} />
                {selectedFile ? (
                  <div className="flex items-center gap-2.5">
                    ...
                    <button className="ml-2 text-muted-foreground hover:text-error transition-colors" onClick={(e) => { e.stopPropagation(); setSelectedFile(null); }}><X size={14} /></button>
                  </div>
                ) : (
                  <>
              ...
              <div>
                ...
                {form.tipo === "Otro" && (
                  <input type="text" value={form.tipoCustom} onChange={(e) => setForm((f) => ({ ...f, tipoCustom: e.target.value }))} placeholder="Nombre del tipo…" className={cn(inputCls, "mt-2")} />
                )}
              </div>
              <div>
                <label className={labelCls}>Nombre del documento <span className="text-error">*</span></label>
                <input type="text" value={form.nombre} onChange={(e) => setForm((f) => ({ ...f, nombre: e.target.value }))} placeholder="Ej: DNI Frente y Dorso" className={inputCls} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.visiblePara} onChange={(e) => setForm((f) => ({ ...f, visiblePara: e.target.value as typeof form.visiblePara }))} className={inputCls}>
                    <option value="staff">Solo staff</option>
                    <option value="inquilino">Inquilino</option>
                    <option value="ambos">Staff e inquilino</option>
                  </select>
                </div>
              </div>
              <div>
                <label className={labelCls}>Notas internas (opcional)</label>
                <input type="text" value={form.notasInternas} onChange={(e) => setForm((f) => ({ ...f, notasInternas: e.target.value }))} placeholder="Observaciones para el staff…" className={inputCls} />
              </div>
            </div>
            <div className="px-6 py-4 border-t border-border flex justify-end gap-2">
              <Button variant="ghost" size="sm" onClick={() => { setShowModal(false); resetForm(); }} disabled={saving}>Cancelar</Button>
