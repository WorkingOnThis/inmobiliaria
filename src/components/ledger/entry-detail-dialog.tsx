...
export function EntryDetailDialog({ entry, onOpenChange, onSave, isSplitContract = false, onBeneficiarioOverride, readOnly = false }: Props) {
  ...
  const [notasImprimir, setNotasImprimir] = useState(false);
  ...
  useEffect(() => {
    if (entry) {
      ...
      setNotasImprimir(entry.notasImprimir);
      setError(null);
      setSaving(false);
  ...
  const notasEditable = !readOnly && entry !== null && entry.estado !== "conciliado" && entry.estado !== "cancelado";
  ...
  async function handleSave() {
    ...
    try {
      if (isEditable) {
        ...
        if (isNaN(montoNum) || montoNum <= 0) { setError("El monto debe ser un número positivo"); setSaving(false); return; }
        if (!descripcion.trim()) { setError("La descripción es obligatoria"); setSaving(false); return; }
        await onSave({
        ...
        // Solo notas — entry auto-generated o read-only en otros campos
        await onSave({ notas: notas.trim() || null, notasImprimir });
      }
      onOpenChange(false);
    } catch (e) {
            ...
            <div className="border-t border-border pt-3 space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Label htmlFor="d-notas-imprimir" className="text-xs font-normal text-muted-foreground">
                    Imprimir en recibo
                  </Label>
                  <Switch
                    id="d-notas-imprimir"
                    checked={notasImprimir}
                    onCheckedChange={setNotasImprimir}
                    disabled={!notasEditable}
                  />
                </div>
              </div>
              <Textarea
                ...
                placeholder="Notas internas o para el recibo…"
