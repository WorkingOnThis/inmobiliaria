...
function ServicioChip({
  ...
  return (
    ...
    >
      ...
      {label}
      {diasLabel}
      {estado === "blocked" && <Lock className="h-2.5 w-2.5 ml-0.5" />}
    </span>
  );
}
...
export function ServicesControlPanel() {
  ...
  return (
    <div className="flex min-h-full flex-col">
      {/* Page header */}
      <div className="flex items-center justify-between gap-4 border-b border-border bg-bg px-6 py-5">
        <div>
          <h1
            className="font-headline text-[1.375rem] font-bold text-on-bg"
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {prop.servicios.map((s) => (
                          <ServicioChip
                            ...
                            onClick={() => {
                              router.push(
                                `/propiedades/${prop.propertyId}?tab=servicios&serviceId=${s.id}`
                              );
                            }}
                          />
                        ))}
